    ...
    def _fill_nan(self, feat, mapped=False):
        ...
        for field in feat:
            ...
            if ftype == 'float':
                ...
                feat[field].fillna(value='' if mapped else '', inplace=True)
            else:
...
class TensorFrame(Dataset):
    ...
    def fromPandasDF(cls, dataframe, dataset):
        ...
        for field in dataframe:
            ...
            if ftype == 'token_seq':
                ...
                raise ValueError(f'Expect field type of {field} be `token_seq`, `float_seq`, `token`, `float`, `text`, but got {ftype}.')
    ...
    def __getitem__(self, idx):
        ...
        for field, value in self.data.items():
            if field == 'text' and isinstance(idx, list):
                ret[field] = [value[_] for _ in idx]
            else:
                ret[field] = value[idx]
        return ret
