...
class Recommender(torch.nn.Module, abc.ABC):
    ...
    def fit(
        ...
        **kwargs
        ...
        if self.config['model']['ckpt_path']:
            self.load_checkpoint_partly(self.config['model']['ckpt_path'])
        ...
        if self.run_mode == 'tune':
            ...
            if len(gpu_list) == 1:
                ...
                raise ValueError("expecting accelerator to be 'dp' or 'ddp'"
                                 f"while get {self.config['train']['accelerator']} instead.")
        else:
    ...
    def load_checkpoint_partly(self, path: str) -> None:
        ckpt = torch.load(path)['parameters']

        u_ckpt = {}
        for k, v in ckpt.items():
            if k.startswith('query_encoder.transformer_layer'):
                u_ckpt[k.replace('query_encoder.transformer_layer.', '')] = v
        self.query_encoder.transformer_layer.load_state_dict(u_ckpt)

        u_ckpt = {}
        for k, v in ckpt.items():
            if k.startswith('query_encoder.position_emb'):
                u_ckpt[k.replace('query_encoder.position_emb.', '')] = v
        self.query_encoder.position_emb.load_state_dict(u_ckpt)

        i_ckpt = {}
        for k, v in ckpt.items():
            if k.startswith('item_encoder.lm_projection'):
                i_ckpt[k.replace('item_encoder.lm_projection.', '')] = v
        self.item_encoder.lm_projection.load_state_dict(i_ckpt)
