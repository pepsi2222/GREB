...
class BaseRetriever(Recommender):
    ...
    def topk(self, batch, k, user_h=None, return_query=False):
        ...
        revised_k = min(k + more, self.item_vector.size(0))
        if self.use_index:
            if isinstance(self.score_func, CosineScorer):
                score, topk_items = self.ann_index.search(
                    torch.nn.functional.normalize(query, dim=1).numpy(), revised_k)
            else:
                score, topk_items = self.ann_index.search(query.numpy(), revised_k)
        else:
            score, topk_items = torch.topk(self.score_func(query, self.item_vector), revised_k)
