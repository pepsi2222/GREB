...
def value2type(key, value):
    ...
    if value is None:
        ...
        if len(value) > 0:
            ...
            arg_args['type'] = int
            arg_args['nargs'] = "*"
            arg_args['help'] = f"List({int.__name__}) type, e.g. '--{key} v1 v2 v3'"
...
def get_default_parser() -> ArgumentParser:
    ...
    group.add_argument('--model', '-m', type=str, default='TE_ID_SASRec', help='model name')
    group.add_argument('--dataset', '-d', type=str, default='', help='dataset name')
    ...
    group.add_argument('--use_dataset_config', type=int, default=0)
    return parser
