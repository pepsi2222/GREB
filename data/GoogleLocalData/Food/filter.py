...
if __name__ == '__main__':
    ...
    with open(os.path.join(work_dir, 'data/GoogleLocalData/Food/rating_dataset', f'{category}.pkl'), 'rb') as f:
        trn_set = pickle.load(f)
        tst_set = pickle.load(f)
        dataset = pickle.load(f)
    print(dataset.item_feat)

    characteristic = json.load(open(f'summarized_review/{category}.json'))
    characteristic = pd.DataFrame(
                        list(characteristic.items()),
                        columns=['gmap_id', 'characteristic']
                    )
    characteristic['gmap_id'] = characteristic['gmap_id'].map(
                                    lambda x: dataset.field2token2idx['gmap_id'].get(x, -1))
    characteristic['characteristic'] = characteristic['characteristic'].map(
    ...
    text_fields = ['name', 'characteristic']
    text = dataset.item_feat['name'] + '. ' + dataset.item_feat['characteristic']
    ...
    with open(os.path.join(work_dir, 'data/GoogleLocalData/Food/rating_dataset', f'filtered_{category}.pkl'), 'wb') as f:
