...
if __name__ == '__main__':
    ...
    with open(os.path.join(work_dir, 'data/GoogleLocalData/review', f'{category}.json'), 'r') as rf:
        while True:
            datum = rf.readline()
            if not datum:
                break
            ...
            try:
                ...
                if datum['gmap_id'] in food_gmap_ids and datum['text'] is not None and is_english(datum['text']):
                    review[datum['gmap_id']].add(clean_text(datum['text']))
    ...
    with open(os.path.join(work_dir, 'data/GoogleLocalData/Food/extracted_review', f'{category}.json'), 'w') as f:
        json.dump(review, f)
    print('Success!')
