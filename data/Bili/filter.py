...
if __name__ == '__main__':
    ...
    with open(os.path.join(work_dir, 'data/Bili/dataset', f'{os.path.basename(category)}.pkl'), 'rb') as f:
        trn_set = pickle.load(f)
        tst_set = pickle.load(f)
        dataset = pickle.load(f)
    print(dataset.item_feat)

    text_fields = ['en_title']
    text = dataset.item_feat['en_title'].map(lambda x: clean_text(x))
    ...
    with open(os.path.join(work_dir, 'data/Bili/dataset', f'filtered_{os.path.basename(category)}.pkl'), 'wb') as f:
