...
if __name__ == '__main__':
    ...
    with open(os.path.join(work_dir, 'data/Goodreads/goodreads_reviews_dedup.json'), 'r') as rf:
        cnt = 0
        st = time.time()
        while True:
            datum = rf.readline()
            ...
            try:
                ...
                if datum['review_text'] is not None and is_english(datum['review_text']):
                    review[datum['book_id']].add(clean_text(datum['review_text']))
            except Exception as e:
                print(e, datum)
                continue
            if (cnt % 100000) == 0:
                et = time.time()
                print(cnt, (et - st) / 3600)
                st = time.time()
            cnt += 1
    ...
    with open(os.path.join(work_dir, 'data/Goodreads/filtered_reviews.json'), 'w') as f:
        json.dump(review, f)
    print('Success!')
