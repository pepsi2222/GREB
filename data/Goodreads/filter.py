...
if __name__ == '__main__':
    ...
    with open(os.path.join(work_dir, 'data/Goodreads', f'Goodreads_rating.pkl'), 'rb') as f:
        trn_set = pickle.load(f)
        tst_set = pickle.load(f)
        dataset = pickle.load(f)
    print(dataset.item_feat)    # 1827820

    # characteristic = json.load(open(f'summarized_review.json'))
    # characteristic = pd.DataFrame(
    #                     list(characteristic.items()),
    #                     columns=['book_id', 'characteristic']
    #                 )
    # characteristic['book_id'] = characteristic['book_id'].map(
    #                                 lambda x: dataset.field2token2idx['book_id'][x])
    ...
    # text_fields = ['title', 'description', 'characteristic']
    text_fields = ['title', 'description']
    text = dataset.item_feat['title'] + '. ' + dataset.item_feat['description'] # + dataset.item_feat['characteristic']
    ...
    with open(os.path.join(work_dir, 'data/Goodreads', f'filtered_Goodreads_rating.pkl'), 'wb') as f:
